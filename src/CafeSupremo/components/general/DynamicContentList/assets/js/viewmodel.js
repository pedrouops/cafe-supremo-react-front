...
define(
  ...
  function (ko, $, compCtx, defaultComponents) {
    'use strict'

    // Keep references to SDK and APIs
    var SitesSDK, renderAPI, caasApi, sectionLayouts

    // Get section layouts
    var sectionLayouts = defaultComponents.components.filter(function (
      category
    ) {
    ...
    var ComponentViewModel = function (args) {
      ...
      self.showNoResults = ko.computed(function () {
        return (
          (self.queryAttempted() || self.viewMode === 'edit') &&
          self.contentComponentIds().length === 0
        )
      })
      ...
      SitesSDK.getProperty(
        'customSettingsData',
        $.proxy(self.updateCustomSettingsData, self)
      )
    ...
    ComponentViewModel.prototype.getDetailPageId = function (contentType) {
      ...
      if (!self.detailPageIds[contentType]) {
        ...
        var pageId =
          contentDetailPageId !== -1 ? contentDetailPageId : detailPageId
    ...
    ComponentViewModel.prototype.getContentItems = function (typeFilter) {
      ...
      var callback = function (searchResults) {
        if (filters.length > 0) {
          var filteredItems = searchResults.items.filter(function (
            searchResultsItem
          ) {
      ...
      if (contentType.length > 0) {
        ...
        compCtx.caasApi.getContentItems(
          contentType,
          options,
          callback,
          errorCallback
    ...
    ComponentViewModel.prototype.getQueryString = function (options) {
      var query = Object.keys(options.searchOptions)
        .filter(function (key) {
        ...
        .map(function (key) {
          return (
            ...
            '=' +
            encodeURIComponent(options.searchOptions[key])
          )
        })
        .join('&')
          ...
          $.each(componentIds, function (index, id) {
            if (typeof id === 'string') {
              try {
                ...
                console.error(
                  'Error removing componentId from pageModel: ',
                  id,
                  err
                )
              }
     ...
     */
    ComponentViewModel.prototype.renderList = function (
      self,
      searchResultsObject
    ) {
      ...
      searchResultsObject.items.forEach(function (searchResultsItem) {
        ...
        if (layoutCategory === undefined) {
          layoutCategory = self.contentLayoutId() || 'Default'
        }
        var componentData = {
          componentId: '',
          ...
          contentViewing: self.viewMode == null ? 'published' : 'draft',
          isCaaSLayout: true,
      ...
      var sectionLayoutCustomSettings = $.extend(
        {},
        seededLayout.initialData.customSettingsData
      )
    ...
    ComponentViewModel.prototype.getLayoutName = function (
      contentType,
      layoutCategory
    ) {
      var layoutName
