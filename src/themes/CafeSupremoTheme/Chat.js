          ...
          <div className="chat-col-4">
            <div className="chat-card">
              <div className="chat-card-body">
                  <div className="chat-card-title">Search the Supremo Content Base       <button id='someButton2' >X</button></div>

                <div className="messages">
                  {
                    this.state.messages.map((message,index) => {
                      if(index % 2 === 0){
                      return (<div className="bubble-left" key={index} >{message}</div>)
                      }
                      else{
                      return  <div className="bubble-right" key={index} >{message}</div>
                      }
              ...
              <div className="chat-card-footer">
                <br/>
                <input type="text" placeholder="Content to search for eg. Coffee" className="form-control" value={this.state.message} onChange={ev => this.setState({message: ev.target.value})}/>
                <br/>
                <button onClick={this.sendMessage} className="btn btn-primary form-control">Search</button>
              </div>
            </div>
          </div>
        </div>
